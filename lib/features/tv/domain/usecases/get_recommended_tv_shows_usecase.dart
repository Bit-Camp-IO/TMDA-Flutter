...
class GetRecommendedTvShowsUseCase {
  ...
  const GetRecommendedTvShowsUseCase(this._tvShowsRepository);

  Future<Either<Failure, List<TvShow>>> call({required int pageNumber, required int tvShowId}) async{
    return await _tvShowsRepository.getRecommendedTvShows(pageNumber: pageNumber, tvShowId: tvShowId);
  }
}
