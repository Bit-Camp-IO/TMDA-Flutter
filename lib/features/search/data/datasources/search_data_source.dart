...
class SearchDataSourceIml extends SearchDataSource {
  ...
  Future<List<MovieSearchModel>> searchForMovie(
    ...
    return List<MovieSearchModel>.from((listOfMovies['results'])
        .map((movies) => MovieSearchModel.fromJson(movies)));
  }
  ...
  Future<List<SearchTvShowModel>> searchForTvShow(
    ...
    return List<SearchTvShowModel>.from((listOfTvShows['results'])
        .map((tvShows) => SearchTvShowModel.fromJson(tvShows)));
  }
  ...
  Future<List<PersonSearchModel>> searchForPerson(
    ...
    return List<PersonSearchModel>.from((listOfPersons['results'])
        .map((person) => PersonSearchModel.fromJson(person)));
  }
}
